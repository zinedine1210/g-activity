...
export default function CardTask({item, project, hide, status}) {
  ...
  return (
    <div id={item.id} className={`-z-20 h-full w-full border-l-8 ${item.is_owner ? "border-green-500":"border-red-500"} bg-white dark:bg-dark rounded-md shadow-md p-3 cursor-pointer ${item.done == true ? hide ? "":"hidden":""}`} onDragOver={e => e.preventDefault()} draggable onDragEnter={e => handlerDragOver(e, item.id)} onDragLeave={e => handlerDragLeave(e, item.id)} onDragStart={(e) => handleDragStart(e, item)} onDrop={(e) => handleDrop(e, item, 'task')} onClick={() => handlerActiveTask()}>
