...
export default function ModalRole({ statename }) {
    ...
    useEffect(() => {
        if (type == "create") {
            ...
            setValue(obj)
            setTypename("Update Role")
        } else {
            setTypename("Role Detail")
            setValue(data)
        }
    }, [type, data])
                                ...
                                value && (
                                    <div className="w-full mt-10 space-y-5 overflow-y-auto flex-1">
                                        ...
                                        <div>
                                            ...
                                            <input type="color" disabled={isDisabledView} value={value.color} required name="color" onChange={e => handlerChange(e.target.value, e.target.name)} className="mt-2 block w-full h-20 placeholder-zinc-400/70 rounded-lg border peer transition-colors invalid:focus:border-red-400 invalid:focus:ring-red-300 invalid:focus:ring-opacity-40 invalid:border-red-200 border-zinc-200 bg-white px-5 py-2 text-zinc-700 focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-dark dark:bg-dark dark:text-white" autoComplete="off" />
                                        </div>
                                    </div>
                                )
                            }
