...
export default function Application({data, t, profileData}) {
  ...
  const handlerCreateNote = async () => {
    ...
    if(result.data.type == "success"){
        ...
        router.push(`/usr/note?id=${result.data.data.id}`)
    }else{
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
  ...
  const handlerCreateMOM = async () => {
    ...
    if(result.data.type == "success"){
        ...
        router.push(`/usr/mom?id=${result.data.data.id}`)
    }else{
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
  ...
  return (
    <>
        ...
        {/* <h1 className="mt-5 mb-10 flex items-center gap-2">
        {t("pinned")}
        <FaThu`btack />
        </h1>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-20">
