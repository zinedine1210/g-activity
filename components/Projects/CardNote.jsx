...
export default function CardNote({ item, profileData }) {
    ...
    const handlerLink = () => {
        navigator.clipboard.writeText(`${hostName}/usr/note?id=${item.id}`).then(() => {
    ...
    const handlerSalinan = async () => {
        ...
        const result = await NoteRepository.postNote({ data: item, xa: JSON.parse(localStorage.getItem("XA")) })
        console.log(result);
        if (result.status == 0) {
            mutate(['notes', 1, item.project_id], cache => {
    ...
    const handlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const result = await NoteRepository.deleteNote({ id: item.id, xa: JSON.parse(localStorage.getItem('XA')) })
                console.log(result);
                if (result.status == 0) {
                    mutate(['notes', 1, item.project_id], cache => {
    ...
    return (
        <div className="w-full relative h-36 group">
            <div className="overflow-hidden relative w-full shadow-lg dark:border dark:border-darkSecondary h-full p-2 rounded-lg">
                <div className="w-6 bg-gradient-to-b z-10 from-blue-600 via-sky-600 to-blue-800 group-hover:from-blue-500 group-hover:via-sky-500 group-hover:to-blue-700 h-full absolute top-0 left-0 bg-black">
                </div>
                <Link href={`/usr/note?id=${item.id}`}>
                    <div className="h-full">
                        <div className="relative">
                            <div className="flex items-center gap-1">
                                <span className="bg-sky-400 flex items-center w-10 h-10 rounded-md justify-center text-lg font-extrabold text-white z-10 font-mono">{item.title != "" ? item.title.charAt(0) : "--"}</span>
                                <div>
                                    <h1 className="font-bold text-sm">{item.title != "" ? item.title.length > 20 ? item.title.substring(0, 20) + "..." : item.title : <span className="text-blue-500">Untitled Note</span>}</h1>
                                    <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-300">Note</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div ref={dropdownRef} className="relative">
                <div className={`${open ? "block" : "hidden"} bg-white dark:bg-darkPrimary rounded-md shadow-lg absolute top-full right-0 z-20`}>
                    <ul className="text-sm">
                        ...
                        {
                            (profileData['_bitws']['delete'] & profileData['_feature']['ga_note']) ? <li className="border-t pt-2">
