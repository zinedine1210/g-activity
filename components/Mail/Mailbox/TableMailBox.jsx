...
export default function TableMailBox({
    ...
    const getDataBoxMail = async (filterUID, filterType) => {
        console.log('filterUID, filterType', filterUID, filterType)
        const result = await CollectionData.getData({ url: `mail/${filterUID}/${filterType}`})
    ...
    useEffect(() => {
        ...
        console.log("tess hash", hash)
        console.log("router query", router.query.uid)
        getDataBoxMail(router.query.uid, hash)
    }, [router.asPath]); // Dependensi untuk memantau perubahan URL
            ...
            <div className="bg-gray-100 mb-6 max-w-full overflow-hidden">
                <ul className="ul-container">
                    {dataMail.map((mail, index) => {
                        ...
                        return (
                            <li key={index} className="flex items-center border-y hover:bg-gray-200 px-2">
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <span className="min-w-64 pr-2 truncate-text">{mail.sender}</span>
                                        <div className="w-full flex flex-col max-w-full">
                                            <span className="truncate-text">{mail.title}</span>
                                            <span className="truncate-multiline text-gray-400 block text-ellipsis overflow-hidden">{truncateText(mail.desc, 100)}</span>
                                        </div>
                                    </div>
