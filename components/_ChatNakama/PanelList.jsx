...
import { nakamaClient, socket, initSocket } from '@components/_Nakama/NakamaClient';
...
export default function PanelList({
    ...
    useEffect(() => {
        console.log("ini init socket")
        console.log("profileDatass", profileData)
    ...
    let optionsChat = [
        {
            ...
            action: (value) => {
                router.push({
                    pathname: "/usr/chat-nakama/contact",
                    query: router.query
    ...
    const handleStartChat = async item => {
        ...
        router.push(`/usr/chat-nakama?roomId=${item.id}`)
    }
                ...
                <header className="w-full border-b-2 border-blue-500 shadow-md px-2 py-2.5">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative w-full">
                            ...
                            <input type="search" value={keyword} onChange={(e) => setKeyword(e.target.value)} className="focus:bg-zinc-300 duration-300 rounded-md outline-none w-full py-3 pl-10 text-sm placeholder:text-zinc-500" placeholder="nakama chat broo" />
                        </div>
                    </div>
                </header>
