import { getTimeAgoFromIsoString } from '@utils/function';
            ...
            <span className="text-sm font-semibold text-gray-900 dark:text-white">{data?.label}</span>
            {
                isContext && (
                    <a href={`#${isContext.id}`} className="border-s-4 block border-teal-800 mt-2 bg-teal-100 w-full rounded-md p-3">
                        <h1 className="text-teal-800 capitalize font-bold text-sm">{isContext.user_id == data.user_id ? "You": context?.dataDetailRoom?.label}</h1>
                        <p className="text-zinc-600 text-sm">{isContext?.msg}</p>
                    </a>
                )
            }
            <p className="text-sm font-normal py-1.5 text-gray-900 dark:text-white">{data?.content?.msg}</p>
            <div className="absolute bottom-1 right-2 flex items-center gap-2 text-black">
                <p className="text-xs">{getTimeAgoFromIsoString(data?.create_time)}</p>
            </div>
        </div>
