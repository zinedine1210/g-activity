...
export default function Navbar(props) {
    ...
    const handlerSave = async () => {
        ...
        if(returnRes.data.type == "success"){
            Swal.fire({
                icon:"success",
                position:"top-end",
            ...
            mutate(['documentation', 1, context.dataDocumentation.project_id], cache => {
                const indexData = findIndex(cache?.data?.data, {id:returnRes.data.data.id})
                cache.data[indexData] = returnRes.data.data
                return cache
            }, false)
        }else{{
            Swal.fire({
